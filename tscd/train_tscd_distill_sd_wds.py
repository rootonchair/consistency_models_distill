...
def parse_args():
    ...
    # ----Trajectory Segmented Consistency Distillation (TSCD) Specific Arguments----
    parser.add_argument(
        '--stage_steps',
        type=int,
        default=None,
        help="The number of optimization steps performs before moving to the next distillation stage."
    )
    parser.add_argument(
        '--stage_segments',
        type=str,
        default="8,4,2,1",
        help="The number of segments that the trajectory is divided into in each stage."
    )
    parser.add_argument(
        "--w_min",
        type=float,
...
def main(args):
    ...
    k_list = [int(i) for i in args.stage_segments.split(',')]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # 3. Calculate the number of segments to divide the trajectory
                k_idx = global_step // args.stage_steps
                if k_idx >= len(k_list):
                    k_idx = len(k_list) - 1
                k = k_list[k_idx]

                # 3.2. Base on the timestep being sampled, calculate the segment end timestep
                segment_steps = noise_scheduler.config.num_train_timesteps // k
                t_b = start_timesteps // segment_steps
                t_b = t_b * segment_steps
                end_timesteps = randint(t_b, timesteps, (bsz,), device=latents.device).long()
